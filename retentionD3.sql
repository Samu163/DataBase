...
SELECT 
    ...
    COUNT(DISTINCT rd.player_id) AS returned_d3,
    ROUND((COUNT(DISTINCT rd.player_id) / COUNT(DISTINCT fs.player_id)) * 100, 2) AS d3_retention_percentage
FROM first_session fs
LEFT JOIN returned_d3 rd ON fs.player_id = rd.player_id
